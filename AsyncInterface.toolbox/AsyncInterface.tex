...
\@x{}%
\@y{\@s{0}%
 We first define the type invariant. ie., the invariant that should be
 satisfied
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 by the spec at all times and that defines what the legal values can be for
 the
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 variables in the \ensuremath{Spec}. Here we define \ensuremath{val} to be
 some value in the set \ensuremath{Data}, \ensuremath{rdy
}}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 and \ensuremath{ack} can be either 0 or 1. It cannot have any other value.
}%
\@xx{}%
...
\@x{}%
\@y{\@s{0}%
 Initially, \ensuremath{val} can be any \ensuremath{Data}, \ensuremath{rdy}
 and \ensuremath{ack} are the same value and can be either 0 or 1.
}%
\@xx{}%
\@x{ Init \.{\defeq} \.{\land} ( val\@s{2.08} \.{\in} Data )}%
\@x{\@s{35.70} \.{\land} ( rdy\@s{0.83} \.{\in} \{ 0 ,\, 1 \} )}%
\@x{\@s{35.70} \.{\land} ( ack \.{=} rdy )}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@x{}%
\@y{\@s{0}%
 \ensuremath{Next \.{\defeq} Send \.{\lor} Recv
}}%
\@xx{}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 \ensuremath{Send \.{\rightarrow}} For send to happen, initially
 \ensuremath{rdy} and \ensuremath{ack} should be the same.
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 The next state of \ensuremath{rdy} should change to !\ensuremath{rdy} and
 \ensuremath{ack} should remain the same
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 Ofcourse, \ensuremath{val} is now the data that we want to send and it will
 be something from set \ensuremath{Data
}}%
\@xx{}%
\@pvspace{8.0pt}%
\@x{ Send \.{\defeq} \.{\land} rdy \.{=} ack}%
\@x{\@s{40.87} \.{\land} val \.{'}\@s{1.25} \.{\in} Data}%
\@x{\@s{40.87} \.{\land} rdy \.{'} \.{=} 1 \.{-} rdy}%
\@x{\@s{40.87} \.{\land} {\UNCHANGED} ack}%
\@pvspace{8.0pt}%
\@x{ Recv\@s{0.84} \.{\defeq} \.{\land} rdy \.{\neq} ack}%
\@x{\@s{40.87} \.{\land} ack \.{'} \.{=} 1 \.{-} ack}%
\@x{\@s{40.87} \.{\land} {\UNCHANGED} {\langle} val ,\, rdy {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Next\@s{1.04} \.{\defeq} Send \.{\lor} Recv}%
\@pvspace{8.0pt}%
 \@x{ Spec\@s{2.51} \.{\defeq} Init \.{\land} [ Next ]_{ {\langle} val ,\, ack
 ,\, rdy {\rangle}}}%
\@x{}\bottombar\@xx{}%
\setboolean{shading}{false}
\begin{lcom}{0}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Nov} 27 11:02:32 \ensuremath{PST} 2014 by \ensuremath{svkodaka
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Thu} \ensuremath{Nov} 27
 10:14:42 \ensuremath{PST} 2014 by \ensuremath{svkodaka
}%
\end{cpar}%
\end{lcom}%
\end{document}
