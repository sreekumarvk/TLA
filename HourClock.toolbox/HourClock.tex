...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* This module defines an hour clock. It is a
 counter which will increment from 1 to 12 and then reverts back to 1.
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* This also allows stturing, ie., the clock
 can reach a state say 11 and remain in that state forever.
\end{cpar}%
\end{lcom}%
...
\@x{}%
\@y{\@s{0}%
 We define the next state in two different ways. One using \ensuremath{{\IF}}
 \ensuremath{\.{\THEN} \.{\ELSE}} condition and the other using
 \ensuremath{MOD} operator
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 Note that individually \ensuremath{HCnxt} and \ensuremath{HCnxt2} are not
 equivalent. This is because in \ensuremath{HCnxt}, \ensuremath{hr\.{'}} will
 become 1 only when \ensuremath{hr} is 12,
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 while in \ensuremath{HCnxt2}, \ensuremath{hr\.{'}} will become 1 when
 \ensuremath{hr} is a multiple of 12.
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 Lets assume that \ensuremath{hr} is set to 15. Then \ensuremath{hr\.{'}}
 will be 16 in \ensuremath{HCnxt} while in \ensuremath{HCnxt2} it will be
 \ensuremath{15\.{\%}12} which is 3.
}%
\@xx{}%
 \@x{ HCnxt \.{\defeq} hr \.{'} \.{=} {\IF} hr \.{\neq} 12 \.{\THEN} hr \.{+}
 1 \.{\ELSE} 1}%
\@x{ HCnxt2 \.{\defeq} hr \.{'} \.{=} ( hr \.{\%} 12 ) \.{+} 1}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 But when \ensuremath{HCnxt} and \ensuremath{HCnxt2} taken together with
 \ensuremath{HCini}, the two statements are equivalent. Here \ensuremath{HC}
 and \ensuremath{HC2} is the statement
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 that defined \ensuremath{HCnxt} and \ensuremath{HCnxt2} taken together with
 \ensuremath{HCini} respectively. Hence \ensuremath{HC} and \ensuremath{HC2}
 are equivalent and is a theorem
}%
\@xx{}%
\@x{ HC \.{\defeq} HCini \.{\land} {\Box} [ HCnxt ]_{ hr}}%
\@x{ HC2 \.{\defeq} HCini \.{\land} {\Box} [ HCnxt2 ]_{ hr}}%
\@pvspace{8.0pt}%
\@x{ {\THEOREM} HC \.{\implies} {\Box} HCini}%
\@x{ {\THEOREM} HC2 \.{\implies} {\Box} HCini}%
\@x{}%
\@y{%
 \ensuremath{{\THEOREM}} \ensuremath{HC \.{\defeq} HC2
}}%
\@xx{}%
\@pvspace{8.0pt}%
\@x{}\bottombar\@xx{}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Nov} 26 13:11:10 \ensuremath{PST} 2014 by \ensuremath{svkodaka
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Wed} \ensuremath{Nov} 26
 11:36:45 \ensuremath{PST} 2014 by \ensuremath{svkodaka
}%
\end{cpar}%
\end{lcom}%
\end{document}
